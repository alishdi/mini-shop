...
function initNamespaceConnection(endpoint) {
    ...
    namespaceSocket.on("connect", () => {
        ...
        namespaceSocket.on("roomList", rooms => {
            ...
            if(rooms.length===0){
                roomsElement.innerHTML=`<li class="contact">
                <div class="wrap">
                    
                    <div class="meta">
                        <p class="name">No Conversaton Yet</p>
...
function getRoomInfo(endpoint, roomName) {
    ...
    namespaceSocket.on("roomInfo", roomInfo => {
        ...
        document.querySelector("#roomName h3").innerText = roomInfo?.description || 'گروهی یافت نشد'
...
socket.on("connect", async () => {
    socket.on("namespacesList", namespacesList => {
        ...
        initNamespaceConnection(namespacesList[1].endpoint)
